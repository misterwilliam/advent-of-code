...
def is_aunt_compatible(aunt, constraints):
  for key, value in aunt.iteritems():
    if key in constraints:
      if key == "cats" or key == "trees":
        # value must greater than constraint
        if value <= constraints[key]:
          return False
      elif key == "pomeranians" or key == "goldfish":
        # value must be smaller than constraint
        if value >= constraints[key]:
          return False
      else:
...
def list_compatible_aunts(aunts, constraints):
  return [i + 1 for i, aunt in enumerate(aunts) if is_aunt_compatible(aunt, constraints)]

def strip_last_char(string):
  return string[:-1]

def load_aunt_data(data):
  aunts = []
  for line in data.split("\n"):
    tokens = line.split()
    aunt = {}
    for i in xrange(2, len(tokens), 2):
      prop = strip_last_char(tokens[i])
      # Last token has no comma so we don't need to strip last char
      if i + 1 != len(tokens) - 1:
        value = strip_last_char(tokens[i + 1])
      else:
        value = tokens[i + 1]
      aunt[prop] = int(value)
    aunts.append(aunt)
...
aunt_data = load_aunt_data(data)
print list_compatible_aunts(aunt_data, constraints)

# TESTS -------------------------

class MyTests(unittest.TestCase):

  def test_is_aunt_compatible(self):
    aunt = {
      "children": 3
    }
    self.assertTrue(is_aunt_compatible(aunt, {}))
    self.assertTrue(is_aunt_compatible(aunt, {"children": 3}))
    self.assertFalse(is_aunt_compatible(aunt, {"children": 4}))

  def test_is_aunt_compatible_with_goldfish(self):
    aunt = {
      "children": 3,
      "goldfish": 7,
    }
    self.assertFalse(is_aunt_compatible(aunt, {"goldfish": 5}))

  def test_list_compatible_aunts(self):
    aunts = [
      {
        "children": 3
      },
    ...
    self.assertEqual(list_compatible_aunts(aunts, {"children": 3}), [1, 2])

  def test_load_aunt_data(self):
    test_data = """Sue 1: vizslas: 8, perfumes: 1, akitas: 3
Sue 2: perfumes: 4, cars: 9, trees: 4"""
    aunt_data = load_aunt_data(test_data)
    self.assertEqual(aunt_data, [
      {
        "vizslas": 8,
        "perfumes": 1,
        "akitas": 3,
